// 4-stage RV32IM pipeline with BRAM-based instruction memory.
// Identical to rv32i_pipeline_4stage_top except:
...
module rv32i_pipeline_4stage_bram_top (
  input logic clk,
  input logic rst_n,
  ...
  output logic [31:0] debug_alu_result
    ...
    // All signal declarations identical to rv32i_pipeline_4stage_top
    ...
    pipe_if_id u_if_id (
        ...
        .pc_out(id_pc), .pc_plus4_out(id_pc_plus4), .instr_out(id_instr),
    ...
    // ID/EX: stall on BRAM miss
    pipe_id_ex u_id_ex (
        .clk(clk), .rst_n(rst_n), .flush(id_ex_flush),
        .stall(id_ex_stall || bram_stall),
    ...
    // EX: forwarding + ALU + branch + CSR + MDU
    ...
    forwarding_unit u_forward (
        ...
        .mw_rd_addr(mw_rd_addr), .mw_reg_write(mw_reg_write),
    ...
    // MW: merged memory + writeback
    dmem u_dmem (.clk(clk), .mem_read(mw_mem_read), .mem_write(mw_mem_write),
