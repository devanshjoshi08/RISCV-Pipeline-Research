// 7-stage RV32IM pipeline with a Next-Line Predictor (NLP) in IF1.
// Identical to rv32i_pipeline_7stage_top EXCEPT for the fetch redirect path.
//
// Motivation: the baseline 7-stage makes its branch prediction in IF2 (using
// the registered PC), so a predicted-taken branch must flush pipe_if1_if2 and
// redirect from IF2 -> a 1-cycle "IF2 bubble" on every correctly predicted
// taken branch. This variant adds a small tagless next-fetch predictor that is
// looked up with the IF1 PC and can redirect in IF1 itself, eliminating the
// bubble when it is correct.
//
// CORRECTNESS / METHODOLOGY:
//   * The NLP is PERFORMANCE-ONLY. It is a shadow of the IF2 gshare/BTB taken
//     prediction; it does not touch the GHR, PHT, BTB, or RAS. The branch
//     predictor's state evolution and its taken/not-taken decisions are
//     therefore bit-identical to the baseline 7-stage, so the misprediction
//     count is identical. Only the cycle count (IF2 bubbles) changes.
//   * A wrong NLP redirect is caught one cycle later in IF2 (which always runs
//     the real predictor) and re-steered before the wrong-path instruction can
//     reach EX2, so architectural state (retired instructions, memory, final
//     results) is unchanged. EX2 branch resolution is the ultimate backstop,
//     exactly as in the baseline.
//
// This isolates the IF2-bubble CPI term (Eq. for B(D)) so the §VI-D
// counterfactual can be measured rather than projected.

import pkg_riscv::*;

module rv32i_pipeline_7stage_nlp_top (
  input logic clk,
  input logic rst_n,
  ...
  output logic [31:0] debug_alu_result
    ...
    // =========================================================================
    // NLP: Next-Line Predictor (tagless next-fetch table), looked up in IF1.
    // =========================================================================
    localparam int NLP_DEPTH = 64;
    localparam int NLP_IDX   = 6;                  // $clog2(NLP_DEPTH)
    localparam int NLP_TAG   = 32 - NLP_IDX - 2;   // PC[31:8] tag
    logic               nlp_valid  [0:NLP_DEPTH-1];
    logic [NLP_TAG-1:0] nlp_tag    [0:NLP_DEPTH-1];
    logic [31:0]        nlp_target [0:NLP_DEPTH-1];

    logic [NLP_IDX-1:0] nlp_idx_if1;
    logic               nlp_hit_if1;
    logic [31:0]        nlp_tgt_if1;
    assign nlp_idx_if1 = if1_pc[NLP_IDX+1:2];
    // Tag check (like the BTB) prevents tagless aliasing: the NLP only fires for
    // the exact PC that trained the entry, not for unrelated instructions that
    // happen to share the low index bits.
    assign nlp_hit_if1 = nlp_valid[nlp_idx_if1] &&
    ...
    // IF2 real prediction (combinational, from registered if2_pc)
    logic if2_real_redirect;
    assign if2_real_redirect = if2_predict_valid && if2_predict_taken;

    // Metadata carried IF1 -> IF2: did the NLP redirect for the instr now in IF2,
    // and to where?
    logic        if2_nlp_redir;
    logic [31:0] if2_nlp_tgt;

    // IF2 verification of the NLP action for the instruction currently in IF2.
    logic        if2_agree;
    logic        if2_correct;        // IF2 must flush IF1/IF2 + re-steer (the bubble)
    logic [31:0] if2_correct_tgt;

    assign if2_agree = if2_nlp_redir && if2_real_redirect &&
                       (if2_predict_target == if2_nlp_tgt);

    always_comb begin
        if (if2_nlp_redir) begin
            // NLP redirected this instruction in IF1
            if (if2_agree) begin
                if2_correct     = 1'b0;             // NLP was right -> no bubble
                if2_correct_tgt = 32'b0;
            end else if (if2_real_redirect) begin
                if2_correct     = 1'b1;             // taken, but wrong target
                if2_correct_tgt = if2_predict_target;
            end else begin
                if2_correct     = 1'b1;             // real says not-taken -> seq
                if2_correct_tgt = if2_pc_plus4;
            end
        end else begin
            // NLP did not redirect (sequential fetched)
            if (if2_real_redirect) begin
                if2_correct     = 1'b1;             // normal IF2 bubble (NLP miss)
                if2_correct_tgt = if2_predict_target;
            end else begin
                if2_correct     = 1'b0;             // sequential is correct
                if2_correct_tgt = 32'b0;
            end
        end
    end

    // EX2-level redirect active? (top-priority PC overrides)
    logic ex2_redirect;
    assign ex2_redirect = ex2_trap || ex2_is_mret || ex2_jal || ex2_jalr ||
                          (ex2_do_branch && !ex2_predicted_taken) ||
                          (ex2_branch && !ex2_branch_taken && ex2_predicted_taken);

    // Suppress prediction-driven fetch redirects during stalls (mirrors the
    // baseline's !pc_stall gating of if2_redirect): flushing pipe_if1_if2 during
    // a stall would reset it to pc=0 and inject spurious predictions, corrupting
    // GHR/RAS state.
    logic if2_correct_g;
    assign if2_correct_g = if2_correct && !pc_stall;

    // NLP drives the next PC only if no higher-priority redirect fires this cycle
    logic nlp_drive;
    assign nlp_drive = nlp_hit_if1 && !ex2_redirect && !if2_correct_g && !pc_stall;
    ...
    always_comb begin
        ...
        else if (if2_correct_g)
            pc_next = if2_correct_tgt;       // IF2 re-steer (NLP miss/mismatch)
        else if (nlp_drive)
            pc_next = nlp_tgt_if1;           // NLP redirect in IF1 (no bubble)
        else
    ...
    // =========================================================================
    // IF1/IF2: registers PC + instruction from icache, plus NLP metadata
    // =========================================================================

    logic if1_if2_flush;
    assign if1_if2_flush = if1_if2_flush_hz || if2_correct_g;
    ...
    // NLP metadata register mirrors pipe_if1_if2 stall/flush behaviour exactly
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n || if1_if2_flush) begin
            if2_nlp_redir <= 1'b0;
            if2_nlp_tgt   <= 32'b0;
        end else if (!if1_if2_stall) begin
            if2_nlp_redir <= nlp_drive;
            if2_nlp_tgt   <= nlp_tgt_if1;
        end
    end

    // NLP training: shadow the IF2 gshare/BTB taken prediction.
    logic [NLP_IDX-1:0] nlp_idx_if2;
    assign nlp_idx_if2 = if2_pc[NLP_IDX+1:2];
    integer ni;
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            for (ni = 0; ni < NLP_DEPTH; ni++) nlp_valid[ni] <= 1'b0;
        end else if (!if1_if2_stall) begin
            if (if2_predict_valid) begin           // only known branches (BTB hit)
                if (if2_predict_taken) begin
                    nlp_valid [nlp_idx_if2] <= 1'b1;
                    nlp_tag   [nlp_idx_if2] <= if2_pc[31:NLP_IDX+2];
                    nlp_target[nlp_idx_if2] <= if2_predict_target;
                end else begin
                    nlp_valid [nlp_idx_if2] <= 1'b0;
                end
            end
        end
    end
