...
\begin{abstract}
Deep pipelines on FPGA fabric suffer a previously unquantified branch-prediction degradation: the global history register (GHR) becomes stale as update latency grows with pipeline depth, inflating misprediction counts even when predictor hardware is unchanged.
We quantify this effect, model it analytically, and eliminate it.
...
A first-principles CPI model achieves $R^2 = 0.977$ across 35 workload--depth data points.
To eliminate Mechanism~B, we propose Speculative GHR Forwarding (SGF), a dual-GHR architecture that updates the history register speculatively at prediction time with checkpoint-based rollback on misprediction.
Measured results show that SGF reduces mispredictions by 31.4\% on CoreMark and 22.7\% on statemate in 7/8-stage pipelines---pushing misprediction counts \emph{below} the 4/5-stage baseline---at a cost of 0.5--0.7\% area with no $F_{\max}$ degradation.
Fifty post-place-and-route runs (10 seeds per variant) establish a two-tier frequency structure (70--74\,MHz shallow, 115--118\,MHz deep; $p < 10^{-48}$, Cohen's $d = 18.5$).
\end{abstract}
...
Yet no published study isolates pipeline depth as the sole independent variable on a common FPGA target.
...
Gonzalez and Horowitz identified this effect in ASIC power-performance simulations \cite{gonzalez1999power}, but it has never been measured on FPGA fabric, where the pipeline-depth landscape differs fundamentally from ASIC.
...
The execute-stage split at depth~6 is the sole partitioning boundary that matters on this fabric.
...
We eliminate it with Speculative GHR Forwarding (SGF), a dual-GHR architecture that speculatively updates the history register at prediction time.
Measured results on the Artix-7 fabric show that SGF reduces mispredictions by 31.4\% on CoreMark and 22.7\% on statemate in 7-stage pipelines, at a cost of 55 LUTs and 63 flip-flops (0.7\% area) with no $F_{\max}$ degradation.
...
Table~\ref{tab:prior} summarizes the experimental scope of prior RISC-V FPGA implementations relative to this work.
SERV \cite{serv2019}, PicoRV32 \cite{picorv32}, VexRiscv \cite{vexriscv}, Ibex \cite{schiavone2017slow}, CVA6 \cite{zaruba2019ariane}, and SweRV EH1 \cite{swerv2019} each present a single pipeline organization optimized for a particular design goal.
Frequency and area numbers reported by these projects are useful contextual references, but cannot be compared directly because ISA support, predictor design, memory architecture, forwarding strategy, and Vivado version differ across all of them.
Prior surveys have cataloged pipelining techniques across RISC processor families \cite{biglari2019pipelining}, and the Rocket Chip generator offers a configurable pipeline \cite{asanovic2016rocket}, but neither isolates pipeline depth as a controlled variable.
...
The crc32 pathology traces to a specific microarchitectural interaction.
The inner loop contains a 99.8\% biased branch (the bit-test conditional).
When this branch is rarely mispredicted, SGF shifts the wrong direction into \texttt{spec\_ghr}.
Because the branch is so biased, the speculative GHR remains polluted for several subsequent predictions until either a correct prediction restores the pattern or an EX2 rollback corrects the state.
In a tightly biased loop with one dominant branch, this creates a brief window of GHR contamination.

A \emph{speculative update confidence filter} would address this pathology: suppress speculative GHR updates for branches whose PHT counter is saturated at the extreme (11 or 00), since these branches are unlikely to change direction and their speculative contribution is redundant.
We do not implement this filter in the current design to maintain architectural simplicity, but note that it would add only a 2-bit comparator per prediction cycle.
The key observation is that SGF's worst case (crc32: +0.096\% branch impact, 0\% CPI impact) is negligible compared to its best case (CoreMark: $-$31.4\% misprediction reduction).
...
\begin{itemize}
\item The 6-stage pipeline is the throughput-optimal depth on this fabric. It captures the full frequency benefit of the execute-stage split while paying a moderate CPI penalty (64.1\,MIPS on CoreMark, 1.88 CoreMark/MHz).
...
To verify that the CPI overhead from deeper pipelining is not an artifact of a specific predictor configuration, we swept the PHT depth across 32, 64, and 128 entries while running Dhrystone on the 5-, 6-, and 7-stage variants (9 combinations total).
Fig.~\ref{fig:predsweep} shows that neither CPI nor misprediction rate changes across any of the 9 configurations.
All three PHT sizes produce identical results: 203 mispredictions out of 400 branches (50.8\%) on every variant.
The CPI penalty from deeper pipelining is driven by increased branch resolution latency rather than by predictor table capacity.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{predictor_sweep.png}
\caption{CPI and misprediction rate on Dhrystone across three PHT sizes (32, 64, 128) and three pipeline depths. All 9 configurations yield identical misprediction counts.}
\label{fig:predsweep}
\end{figure}
...
To establish generality across predictor architectures, we replaced the gshare predictor with a bimodal predictor that indexes the PHT using only the PC (no global history register).
Fig.~\ref{fig:predarch} compares the two architectures on Dhrystone.
The bimodal predictor produces nearly identical CPI values (1.48, 1.68, 1.85 for depths 5, 6, 7 respectively---matching gshare exactly) and misprediction counts (202 vs.\ 203).
...
To test whether the 7-stage IF split becomes beneficial with synchronous memory, we synthesized BRAM variants of the 5-, 6-, and 7-stage pipelines using a registered-output instruction memory that infers Block RAM.
Fig.~\ref{fig:bram} shows the results.

With BRAM, the 5-stage achieves 73.7\,MHz, the 6-stage 121.5\,MHz, and the 7-stage 120.9\,MHz.
...
Power estimates are obtained from SAIF-annotated post-simulation switching activity analysis in Vivado for the 5-, 6-, and 7-stage variants.
Fig.~\ref{fig:power} shows the on-chip power breakdown.
Total power is nearly identical across all three variants: 0.299\,W (5-stage), 0.298\,W (6-stage), and 0.300\,W (7-stage).
Static power is constant at 0.069\,W, as expected since it depends on device characteristics rather than design activity.
Dynamic power is also nearly identical ($\sim$0.230\,W), indicating that the pipeline register overhead of deeper variants is offset by shorter combinational paths that reduce glitching \cite{chandrakasan1992low}.
Pipeline depth has negligible impact on power for this class of design on this fabric, making frequency and CPI the dominant efficiency factors.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{power_comparison.png}
\caption{On-chip power breakdown (dynamic and static) for the 5-, 6-, and 7-stage variants on Artix-7. Power is approximately constant across pipeline depths.}
\label{fig:power}
\end{figure}

Fig.~\ref{fig:efficiency} shows the power efficiency in MIPS per watt.
The 6-stage design leads at 237\,MIPS/W, followed by the 7-stage at 205\,MIPS/W and the 5-stage at 159\,MIPS/W.
Since power is approximately constant across variants, efficiency tracks throughput directly, reinforcing the 6-stage as the most efficient design point on this Artix-7 fabric.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{efficiency_comparison.png}
\caption{Power efficiency (MIPS/W) for the 5-, 6-, and 7-stage variants.}
...
For practitioners designing RV32IM-class soft cores on Artix-7: the 6-stage pipeline is the throughput-optimal depth, the classic 5-stage is a dominated design point, and any pipeline with 3 or more stages of predictor update latency should implement SGF.
