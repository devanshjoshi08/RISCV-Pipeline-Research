// 8-stage RV32IM pipeline: IF1 -> IF2 -> ID -> EX1 -> EX2 -> MEM1 -> MEM2 -> WB
// IF is split into IF1 (PC gen, icache read) and IF2 (branch prediction).
// MEM is split into MEM1 (present address to dmem, combinational read) and
// MEM2 (registered dmem output, available for forwarding including loads).
// Branch resolution in EX2 -> 4-cycle mispredict penalty (flush IF1, IF2, ID, EX1).
// IF2 prediction taken -> 1-cycle bubble (flush pipe_if1_if2).
// Forwarding: 3 explicit sources (EX2 -> EX1, MEM1 -> EX1, MEM2 -> EX1).
//   WB writeback data reaches regfile in time for EX1 fresh-read bypass.
//   EX2 and MEM1 exclude loads (data not ready). MEM2 includes loads.
// Load-use: stall when EX2 OR MEM1 has a load and EX1 depends on it.
//   A load immediately followed by dependent gives 2 stall cycles naturally.
// MDU remains multi-cycle (2-cycle multiply, 32-cycle divide), stalls pipeline.

import pkg_riscv::*;

module rv32i_pipeline_8stage_sgf_top (
  input logic clk,
  input logic rst_n,
  ...
  output logic [31:0] debug_alu_result
    ...
    logic        pc_stall, if1_if2_stall, if_id_stall, id_ex1_stall, ex1_ex2_stall;
    logic        if1_if2_flush_hz, if_id_flush, id_ex1_flush, ex1_ex2_flush;
    logic [1:0]  forward_a, forward_b;

    // IF1 stage
    logic [31:0] if1_pc, if1_pc_plus4, if1_instr;
    logic [31:0] pc_next;
    logic        icache_hit;
    logic [31:0] imem_raw_instr, icache_mem_addr;

    // IF1/IF2 outputs
    logic [31:0] if2_pc, if2_pc_plus4, if2_instr;

    // IF2 stage (branch prediction)
    logic        if2_predict_taken;
    logic [31:0] if2_predict_target;
    logic        if2_predict_valid;
    logic        if2_redirect; // IF2 prediction causes PC redirect

    // GHR checkpoint forwarding (SGF: speculative GHR from prediction time)
    logic [5:0]  if2_ghr_checkpoint;
    logic [5:0]  id_ghr_checkpoint;
    logic [5:0]  ex1_ghr_checkpoint;
    logic [5:0]  ex2_ghr_checkpoint;
    ...
    // EX2/MEM1 outputs
    logic [31:0] mem1_pc_plus4, mem1_alu_result, mem1_rs2_data;
    logic [4:0]  mem1_rd_addr;
    logic [2:0]  mem1_funct3;
    logic        mem1_reg_write, mem1_mem_read, mem1_mem_write, mem1_mem_to_reg;
    logic        mem1_jal, mem1_jalr;
    logic        mem1_is_csr;
    logic [31:0] mem1_csr_rdata;

    // MEM1 stage
    logic [31:0] mem1_read_data;

    // MEM1/MEM2 outputs
    logic [31:0] mem2_pc_plus4, mem2_alu_result, mem2_mem_read_data;
    logic [4:0]  mem2_rd_addr;
    logic [2:0]  mem2_funct3;
    logic        mem2_reg_write, mem2_mem_to_reg, mem2_mem_read;
    logic        mem2_jal, mem2_jalr;
    logic        mem2_is_csr;
    logic [31:0] mem2_csr_rdata;
    ...
    logic mem1_retire;
    // Exclude writes to x0 (NOP bubbles from IF2 redirect) from retirement count
    assign mem1_retire = (mem1_reg_write && mem1_rd_addr != 5'b0) ||
                         mem1_mem_write || mem1_mem_read ||
                         mem1_jal || mem1_jalr;

    // IF1: PC generation + icache read (single icache, combinational output)

    assign if1_pc_plus4 = if1_pc + 32'd4;

    // IF2 prediction redirect: when IF2 predicts taken, IF1 has already
    // fetched the wrong-path sequential instruction. Must flush pipe_if1_if2.
    // CRITICAL: gate with !pc_stall -- during MDU/load-use stalls, predictions
    // must be suppressed to prevent predictor state corruption (GHR/RAS).
    assign if2_redirect = if2_predict_valid && if2_predict_taken && !pc_stall;
    ...
    always_comb begin
        if (ex2_trap)
            ...
            pc_next = if2_predict_target;
        else
            pc_next = if1_pc_plus4;
    end
    ...
    // Single icache + imem, read in IF1
    imem u_imem (
    ...
    logic if1_if2_flush;
    assign if1_if2_flush = if1_if2_flush_hz || if2_redirect;

    pipe_if1_if2 u_if1_if2 (
        .clk         (clk),
        .rst_n       (rst_n),
        .stall       (if1_if2_stall),
        .flush       (if1_if2_flush),
        .pc_in       (if1_pc),
        .pc_plus4_in (if1_pc_plus4),
        .instr_in    (if1_instr),
        .pc_out      (if2_pc),
        .pc_plus4_out(if2_pc_plus4),
        .instr_out   (if2_instr)
    );
    ...
    branch_predictor_sgf u_bp (
        .clk            (clk),
        .rst_n          (rst_n),
        .pc_if          (if2_pc),
        .predict_taken  (if2_predict_taken),
        .predict_target (if2_predict_target),
        .predict_valid  (if2_predict_valid),
        .ghr_checkpoint_out (if2_ghr_checkpoint),
        .ras_push_en    (id_ras_push && !if_id_flush && !if_id_stall),
        ...
        .ghr_checkpoint_in  (ex2_ghr_checkpoint),
        .flush          (ex2_mispredict || ex2_jal || ex2_jalr),
        .flush_ras_ptr  (ex2_ras_ptr),
        .flush_ghr      (6'b0),  // committed_ghr restored internally by SGF predictor
    ...
    pipe_if_id u_if2_id (
        ...
        .flush             (if_id_flush),
        .pc_in             (if2_pc),
        .pc_plus4_in       (if2_pc_plus4),
        .instr_in          (if2_instr),
        .predict_taken_in  (if2_predict_taken && if2_predict_valid),
        .pc_out            (id_pc),
    ...
    // SGF: GHR checkpoint IF2 -> ID (standalone register, not in pipe_if_id)
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n)
            id_ghr_checkpoint <= 6'b0;
        else if (if_id_flush)
            id_ghr_checkpoint <= 6'b0;
        else if (!if_id_stall)
    ...
    // SGF: GHR checkpoint ID -> EX1 (standalone register, not in pipe_id_ex)
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n)
            ex1_ghr_checkpoint <= 6'b0;
        else if (id_ex1_flush)
            ex1_ghr_checkpoint <= 6'b0;
        else if (!id_ex1_stall)
            ex1_ghr_checkpoint <= id_ghr_checkpoint;
    end

    // EX1: forwarding + operand select

    // EX2 forwarding result (1 ahead, excludes loads)
    ...
    // MEM1 forwarding result (2 ahead, excludes loads -- dmem data not ready)
    logic [31:0] mem1_fwd_result;
    always_comb begin
        if (mem1_jal || mem1_jalr) mem1_fwd_result = mem1_pc_plus4;
        else if (mem1_is_csr)      mem1_fwd_result = mem1_csr_rdata;
        else                       mem1_fwd_result = mem1_alu_result;
    end

    // MEM2 forwarding result (3 ahead, includes loads)
    logic [31:0] mem2_fwd_result;
    always_comb begin
        if (mem2_jal || mem2_jalr) mem2_fwd_result = mem2_pc_plus4;
        else if (mem2_mem_read)    mem2_fwd_result = mem2_mem_read_data;
        else if (mem2_is_csr)      mem2_fwd_result = mem2_csr_rdata;
        else                       mem2_fwd_result = mem2_alu_result;
    end
    ...
    // Fresh regfile read with WB bypass (WB is 4 ahead, writes to regfile this cycle)
    ...
    always_comb begin
        case (forward_a)
            2'b01:   ex1_fwd_rs1 = ex2_fwd_result;   // from EX2
            2'b10:   ex1_fwd_rs1 = mem1_fwd_result;   // from MEM1
            2'b11:   ex1_fwd_rs1 = mem2_fwd_result;   // from MEM2
            default: ex1_fwd_rs1 = ex1_rf_rs1;        // regfile (with WB bypass)
        endcase

        case (forward_b)
            2'b01:   ex1_fwd_rs2 = ex2_fwd_result;
            2'b10:   ex1_fwd_rs2 = mem1_fwd_result;
            2'b11:   ex1_fwd_rs2 = mem2_fwd_result;
    ...
    // SGF: GHR checkpoint EX1 -> EX2 (standalone register, not in pipe_ex1_ex2)
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n)
            ex2_ghr_checkpoint <= 6'b0;
        else if (ex1_ex2_flush)
            ex2_ghr_checkpoint <= 6'b0;
        else if (!mdu_stall)
            ex2_ghr_checkpoint <= ex1_ghr_checkpoint;
    end

    // EX2: ALU + branch + CSR + MDU (identical to 6/7-stage)
    ...
    csr_unit u_csr (
        ...
        .retire_en(mem1_retire), .branch_en(ex2_branch), .mispredict_en(ex2_mispredict)
    );
    ...
    // Hazard detection (8-stage: flushes 4 stages on mispredict, load-use
    // checks both EX2 and MEM1)
    ...
    pipe_ex_mem u_ex2_mem1 (
        ...
        .reg_write_out(mem1_reg_write), .mem_read_out(mem1_mem_read),
        .mem_write_out(mem1_mem_write), .mem_to_reg_out(mem1_mem_to_reg),
        .jal_out(mem1_jal), .jalr_out(mem1_jalr),
        .pc_plus4_out(mem1_pc_plus4), .alu_result_out(mem1_alu_result),
        .rs2_data_out(mem1_rs2_data), .rd_addr_out(mem1_rd_addr), .funct3_out(mem1_funct3),
        .is_csr_out(mem1_is_csr), .csr_rdata_out(mem1_csr_rdata)
    );

    // MEM1: present address to dmem (combinational read, clocked write)

    dmem u_dmem (
        .clk(clk), .mem_read(mem1_mem_read), .mem_write(mem1_mem_write),
        .funct3(mem1_funct3), .addr(mem1_alu_result),
        .write_data(mem1_rs2_data), .read_data(mem1_read_data)
    );

    // MEM1/MEM2: registers dmem read data and control signals

    pipe_mem1_mem2 u_mem1_mem2 (
        .clk(clk), .rst_n(rst_n),
        .reg_write_in(mem1_reg_write), .mem_to_reg_in(mem1_mem_to_reg),
        .mem_read_in(mem1_mem_read),
        .jal_in(mem1_jal), .jalr_in(mem1_jalr),
        .pc_plus4_in(mem1_pc_plus4), .alu_result_in(mem1_alu_result),
        .mem_read_data_in(mem1_read_data),
        .rd_addr_in(mem1_rd_addr), .funct3_in(mem1_funct3),
        .is_csr_in(mem1_is_csr), .csr_rdata_in(mem1_csr_rdata),
        .reg_write_out(mem2_reg_write), .mem_to_reg_out(mem2_mem_to_reg),
        .mem_read_out(mem2_mem_read),
        .jal_out(mem2_jal), .jalr_out(mem2_jalr),
        .pc_plus4_out(mem2_pc_plus4), .alu_result_out(mem2_alu_result),
        .mem_read_data_out(mem2_mem_read_data),
        .rd_addr_out(mem2_rd_addr), .funct3_out(mem2_funct3),
        .is_csr_out(mem2_is_csr), .csr_rdata_out(mem2_csr_rdata)
    );

    // MEM2/WB (reuses pipe_mem_wb register)

    pipe_mem_wb u_mem2_wb (
        .clk(clk), .rst_n(rst_n),
        .reg_write_in(mem2_reg_write), .mem_to_reg_in(mem2_mem_to_reg),
        .jal_in(mem2_jal), .jalr_in(mem2_jalr),
        .pc_plus4_in(mem2_pc_plus4), .alu_result_in(mem2_alu_result),
        .mem_read_data_in(mem2_mem_read_data), .rd_addr_in(mem2_rd_addr),
        .is_csr_in(mem2_is_csr), .csr_rdata_in(mem2_csr_rdata),
