...
The effect has never been measured on FPGAs, where the pipeline-depth landscape differs fundamentally from ASIC and where a lightweight, in-order-compatible solution is needed.
...
While speculative history updates exist in complex ASIC out-of-order processors, we present the first lightweight, dual-GHR checkpointing architecture designed for resource-constrained, in-order soft-core processors on FPGA fabric.
...
\textbf{Risk assessment.} SGF's worst case (crc32: +167 mispredictions, 0\% CPI impact) is bounded and predictable: it occurs only on branches with $>$99\% bias in tight loops. No workload in our suite shows CPI degradation from SGF. The worst-case absolute cost (167 extra flush cycles) is three orders of magnitude smaller than SGF's best-case savings (159{,}932 fewer flush cycles on CoreMark).
