...
// 6-stage RV32IM pipeline with Speculative GHR Forwarding (SGF).
// Adapted from rv32i_pipeline_top.sv.
// Changes vs baseline:
//   1. branch_predictor → branch_predictor_sgf (dual-GHR with checkpoint)
//   2. GHR checkpoint forwarded IF → ID → EX1 → EX2 via standalone registers
//   3. flush_ghr tied to 6'b0 (committed_ghr restored internally by SGF predictor)

module rv32i_pipeline_sgf_top (
  input logic clk,
  input logic rst_n,
  ...
  output logic [31:0] debug_alu_result
    ...
    // SGF: GHR checkpoint forwarding chain (IF → ID → EX1 → EX2)
    logic [5:0]  if_ghr_checkpoint;
    ...
    branch_predictor_sgf u_bp (
        ...
        .ghr_checkpoint_out (if_ghr_checkpoint),
    ...
    // SGF: GHR checkpoint IF → ID (standalone register, not in pipe_if_id)
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n)
            ...
            id_ghr_checkpoint <= if_ghr_checkpoint;
    end
