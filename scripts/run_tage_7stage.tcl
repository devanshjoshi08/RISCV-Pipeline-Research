# run_tage_7stage.tcl - measured tage-predictor evaluation.
# Runs CoreMark, statemate, aha-mont64, crc32 on the 7-stage baseline-tage
# and SGF-tage variants (XSim), then synthesizes both for F_max/area.
# Tests whether the tage's GHR-indexed components (global PHT + chooser)
...
#   source C:/Users/Joshi/OneDrive/Documents/GitHub/RISCV-RV32IM-Processor/scripts/run_tage_7stage.tcl
# Output: tage_results.log  (benchmark counts) and tage_synth_results.log
#
...
# the tage RTL has a bug and the misprediction numbers are meaningless -- fix
...
set work_root "D:/RISCV-Vivado/tage"
set log_file "$project_dir/tage_results.log"
set synth_log "$project_dir/tage_synth_results.log"
...
set variants [list \
  [list "base" "$rtl_dir/branch_predictor_tage.sv"     "$rtl7_dir/rv32i_pipeline_7stage_tage_top.sv"     "rv32i_pipeline_7stage_tage_top"] \
  [list "sgf"  "$rtl_dir/branch_predictor_tage_sgf.sv" "$rtl7_dir/rv32i_pipeline_7stage_tage_sgf_top.sv" "rv32i_pipeline_7stage_tage_sgf_top"] ]
...
puts $fd "=== TAGE predictor: baseline vs SGF (7-stage) ==="
...
foreach v $variants {
  ...
  foreach b $benches {
    ...
    set proj "tage_${vlabel}_${blabel}"
...
puts $sf "=== TAGE synthesis (7-stage) ==="
...
foreach v $variants {
  ...
  for {set r 1} {$r <= 3} {incr r} {
    ...
    set proj "tagesyn_${vlabel}_r$r"; set work "$work_root/$proj"
    create_project $proj $work -part $part -force
    add_files -norecurse [concat $shared_rtl [list $pred $top] $rtl7_units]
    set_property file_type SystemVerilog [get_files *.sv]
...
puts "=== tage evaluation complete: $log_file , $synth_log ==="
