// 5-stage RV32IM pipeline with BRAM-based instruction memory.
// Identical to rv32i_pipeline_5stage_top except:
//   - imem replaced with imem_bram (synchronous read, 1-cycle latency)
//   - icache replaced with icache_bram (valid_o for stall on miss)
//   - Pipeline stalls when icache_bram.valid_o == 0

import pkg_riscv::*;

module rv32i_pipeline_5stage_bram_top (
  input logic clk,
  input logic rst_n,
  ...
  output logic [31:0] debug_alu_result
    ...
    // All signal declarations identical to rv32i_pipeline_5stage_top
    ...
    logic        icache_valid;  // BRAM: replaces icache_hit
    ...
    // BRAM stall: freeze entire pipeline when icache misses
    logic bram_stall;
    ...
    always_comb begin
        if (ex_trap)                                     pc_next = mtvec;
        else if (ex_is_mret)                             pc_next = mepc_out;
        else if (ex_jal)                                 pc_next = ex_branch_target;
        else if (ex_jalr)                                pc_next = ex_jalr_target;
        else if (ex_do_branch && !ex_predicted_taken)    pc_next = ex_branch_target;
        else if (ex_branch && !ex_branch_taken && ex_predicted_taken) pc_next = ex_pc_plus4;
        else if (if_predict_valid && if_predict_taken)    pc_next = if_predict_target;
    ...
    branch_predictor u_bp (
        .clk(clk), .rst_n(rst_n), .pc_if(if_pc),
        .predict_taken(if_predict_taken), .predict_target(if_predict_target),
        .predict_valid(if_predict_valid),
        .ras_push_en(id_ras_push && !if_id_flush), .ras_push_addr(id_pc_plus4),
        .update_en(ex_branch || ex_jal), .update_pc(ex_pc),
        .actual_taken(ex_branch_taken || ex_jal), .actual_target(ex_branch_target),
        .update_type(bp_update_type),
        .flush(ex_mispredict || ex_jal || ex_jalr), .flush_ras_ptr(ex_ras_ptr),
        .ras_ptr_out(ras_ptr_current)
    );

    // PC: stall on BRAM miss OR hazard stall
    pc u_pc (.clk(clk), .rst_n(rst_n),
             .pc_write(!pc_stall && !bram_stall),
             .pc_next(pc_next), .pc_out(if_pc));

    // BRAM instruction memory (synchronous read)
    imem_bram u_imem (.clk(clk), .addr(icache_mem_addr), .instr(imem_raw_instr),
                      .data_addr(32'b0), .data_out());

    // BRAM-aware icache
    icache_bram u_icache (.clk(clk), .rst_n(rst_n), .addr(if_pc),
                          .instr(if_instr), .valid_o(icache_valid),
                          .mem_addr(icache_mem_addr), .mem_data(imem_raw_instr));

    assign debug_pc    = if_pc;
    assign debug_instr = id_instr;

    // IF/ID: stall on BRAM miss
    pipe_if_id u_if_id (
        .clk(clk), .rst_n(rst_n),
        .stall(if_id_stall || bram_stall), .flush(if_id_flush),
        .pc_in(if_pc), .pc_plus4_in(if_pc_plus4), .instr_in(if_instr),
        .predict_taken_in(if_predict_taken && if_predict_valid),
        .pc_out(id_pc), .pc_plus4_out(id_pc_plus4),
        .instr_out(id_instr), .predict_taken_out(id_predict_taken)
    );

    // ID (identical)
    assign id_opcode = id_instr[6:0]; assign id_rd = id_instr[11:7];
    assign id_funct3 = id_instr[14:12]; assign id_rs1 = id_instr[19:15];
    assign id_rs2 = id_instr[24:20]; assign id_funct7 = id_instr[31:25];
    assign id_funct12 = id_instr[31:20];
    control u_control (.opcode(id_opcode), .funct3(id_funct3), .funct7(id_funct7),
        .funct12(id_funct12), .reg_write(id_reg_write), .mem_read(id_mem_read),
        .mem_write(id_mem_write), .mem_to_reg(id_mem_to_reg), .alu_src(id_alu_src),
        .branch(id_branch), .jal(id_jal), .jalr(id_jalr), .lui(id_lui), .auipc(id_auipc),
        .imm_type(id_imm_type), .alu_op(id_alu_op), .is_mext(id_is_mext), .mdu_op(id_mdu_op),
        .csr_op(id_csr_op), .csr_zimm(id_csr_zimm), .is_ecall(id_is_ecall),
        .is_ebreak(id_is_ebreak), .is_mret(id_is_mret), .illegal_instr(id_illegal_instr));
    imm_gen u_imm_gen (.instr(id_instr), .imm_type(id_imm_type), .imm(id_imm));
    regfile u_regfile (.clk(clk), .rst_n(rst_n), .we(wb_reg_write),
        .rs1_addr(id_rs1), .rs2_addr(id_rs2), .rd_addr(wb_rd_addr),
        .rd_data(wb_rd_data), .rs1_data(id_rs1_data), .rs2_data(id_rs2_data));
    ...
    // ID/EX
    pipe_id_ex u_id_ex (
        .clk(clk), .rst_n(rst_n), .flush(id_ex_flush), .stall(id_ex_stall || bram_stall),
        ...
        .alu_op_in(id_alu_op), .pc_in(id_pc), .pc_plus4_in(id_pc_plus4),
        ...
        .mem_to_reg_out(ex_mem_to_reg), .alu_src_out(ex_alu_src), .branch_out(ex_branch),
        .jal_out(ex_jal), .jalr_out(ex_jalr), .lui_out(ex_lui), .auipc_out(ex_auipc),
        .alu_op_out(ex_alu_op), .pc_out(ex_pc), .pc_plus4_out(ex_pc_plus4),
    ...
    // EX: forwarding + ALU + branch + CSR + MDU (identical to non-BRAM)
    ...
    forwarding_unit u_forward (.ex_rs1_addr(ex_rs1_addr), .ex_rs2_addr(ex_rs2_addr),
    ...
    assign ex_rf_rs1 = (ex_rs1_addr == 5'b0) ? 32'b0 :
                       (wb_reg_write && wb_rd_addr == ex_rs1_addr) ? wb_rd_data :
                       u_regfile.regs[ex_rs1_addr];
    assign ex_rf_rs2 = (ex_rs2_addr == 5'b0) ? 32'b0 :
                       (wb_reg_write && wb_rd_addr == ex_rs2_addr) ? wb_rd_data :
                       u_regfile.regs[ex_rs2_addr];
    always_comb begin
        case (forward_a)
            2'b01:   ex_fwd_rs1 = mem_fwd_result;
            2'b10:   ex_fwd_rs1 = wb_rd_data;
            default: ex_fwd_rs1 = ex_rf_rs1;
        endcase
        case (forward_b)
            2'b01:   ex_fwd_rs2 = mem_fwd_result;
            2'b10:   ex_fwd_rs2 = wb_rd_data;
    ...
    alu u_alu (.a(ex_alu_a), .b(ex_alu_b), .op(ex_alu_op),
               .result(ex_alu_result), .zero(ex_alu_zero));
    logic mdu_stall;
    assign mdu_stall = ex_is_mext && !mdu_valid;
    assign mdu_start = ex_is_mext && !mdu_busy && !mdu_valid;
    mdu u_mdu (.clk(clk), .rst_n(rst_n), .start(mdu_start), .op(ex_mdu_op),
               .rs1(ex_fwd_rs1), .rs2(ex_fwd_rs2), .result(mdu_result),
               .busy(mdu_busy), .valid(mdu_valid));
    ...
    csr_unit u_csr (.clk(clk), .rst_n(rst_n), .addr(ex_csr_addr),
        .op(ex_trap ? CSR_NONE : ex_csr_op), .wdata(ex_csr_wdata), .rdata(csr_rdata),
        .trap_en(ex_trap), .trap_cause(ex_trap_cause), .trap_pc(ex_pc), .trap_val(ex_trap_val),
        .mtvec_out(mtvec), .mret_en(ex_is_mret), .mepc_out(mepc_out),
        .mstatus_mie(mstatus_mie), .retire_en(mem_retire),
        .branch_en(ex_branch), .mispredict_en(ex_mispredict));
    always_comb begin
    ...
    branch_unit u_branch (.funct3(ex_funct3), .rs1_data(ex_fwd_rs1),
                          .rs2_data(ex_fwd_rs2), .taken(ex_branch_taken));
    assign ex_do_branch = ex_branch & ex_branch_taken;
    assign debug_alu_result = ex_alu_result;

    // Hazard unit
    hazard_unit u_hazard (.mem_mem_read(mem_mem_read), .mem_rd_addr(mem_rd_addr),
        .ex_rs1_addr(ex_rs1_addr), .ex_rs2_addr(ex_rs2_addr),
        .branch_taken(ex_mispredict), .jal_ex(ex_jal), .jalr_ex(ex_jalr),
        .mdu_busy(mdu_stall), .trap_flush(ex_trap), .mret_flush(ex_is_mret),
        .pc_stall(pc_stall), .if_id_stall(if_id_stall), .id_ex_stall(id_ex_stall),
        .if_id_flush(if_id_flush), .id_ex_flush(id_ex_flush));

    // EX/MEM
    logic ex_suppress;
    assign ex_suppress = ex_trap || mdu_stall;
    pipe_ex_mem u_ex_mem (.clk(clk), .rst_n(rst_n),
        ...
        .mem_to_reg_in(ex_mem_to_reg), .jal_in(ex_jal), .jalr_in(ex_jalr),
    ...
    // MEM/WB
    pipe_mem_wb u_mem_wb (.clk(clk), .rst_n(rst_n),
