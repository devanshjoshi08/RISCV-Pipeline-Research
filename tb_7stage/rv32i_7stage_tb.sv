...
module rv32i_7stage_tb;
  ...
  // Halt detection: look for the "jal x0, 0" instruction (0x0000006F)
  // sitting in the ID stage (debug_instr). Once we see it repeatedly,
  // the program has reached its infinite halt loop. This is immune to
  // branch-loop PC patterns that fool history-based detection.
  int halt_count;

  always_ff @(posedge clk) begin
    if (!rst_n)
      halt_count <= 0;
    else if (debug_instr == 32'h0000006F)
  ...
  initial begin
    pass_count = 0;
    fail_count = 0;
    $display("=== 7-Stage Pipeline Testbench ===");
    rst_n = 0;
