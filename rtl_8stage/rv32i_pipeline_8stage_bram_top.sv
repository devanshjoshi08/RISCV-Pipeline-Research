// 8-stage RV32IM pipeline (BRAM variant): IF1 -> IF2 -> ID -> EX1 -> EX2 -> MEM1 -> MEM2 -> WB
// Uses imem_bram + icache_bram instead of imem + icache for BRAM instruction memory.
...
// 8-stage pipeline with BRAM instruction memory.
module rv32i_pipeline_8stage_bram_top (
  input logic clk,
  input logic rst_n,
  ...
  output logic [31:0] debug_alu_result
    ...
    logic        bram_stall;
    assign bram_stall = !icache_valid;
    ...
    // =========================================================================
    // IF1: PC generation + icache read (BRAM variant)
    // =========================================================================
    ...
    assign if2_redirect = if2_predict_valid && if2_predict_taken;
    ...
    pc u_pc (
        .clk      (clk),
        .rst_n    (rst_n),
        .pc_write (!pc_stall && !bram_stall),
        .pc_next  (pc_next),
        .pc_out   (if1_pc)
    );

    // BRAM instruction memory (synchronous read)
    imem_bram u_imem (
        .clk   (clk),
    ...
    pipe_if1_if2 u_if1_if2 (
        .clk         (clk),
        .rst_n       (rst_n),
        .stall       (if1_if2_stall || bram_stall),
        .flush       (if1_if2_flush),
        .pc_in       (if1_pc),
    ...
    pipe_id_ex u_id_ex1 (
        .clk(clk), .rst_n(rst_n), .flush(id_ex1_flush), .stall(id_ex1_stall || bram_stall),
