// MEM1/MEM2 pipeline register for 8-stage pipeline.
// MEM1 presents address to dmem and performs the write. Combinational read data
// from dmem is captured here for MEM2 (where load data becomes available for forwarding).

module pipe_mem1_mem2 (
  input logic clk, rst_n,
  input logic reg_write_in, mem_to_reg_in, mem_read_in,
  input logic jal_in, jalr_in,
  input logic [31:0] pc_plus4_in, alu_result_in, mem_read_data_in,
  input logic [4:0] rd_addr_in,
  input logic [2:0] funct3_in,
  input logic is_csr_in,
  input logic [31:0] csr_rdata_in,

  output logic reg_write_out, mem_to_reg_out, mem_read_out,
  output logic jal_out, jalr_out,
  output logic [31:0] pc_plus4_out, alu_result_out, mem_read_data_out,
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      pc_plus4_out <= 0; alu_result_out <= 0; mem_read_data_out <= 0;
