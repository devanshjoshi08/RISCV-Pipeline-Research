// 5-stage RV32IM pipeline: IF → ID → EX → MEM → WB
...
module rv32i_pipeline_5stage_top (
  ...
  output logic [31:0] debug_alu_result
    ...
    // Fresh register file reads with WB bypass (handles stale pipe_id_ex values
    ...
    csr_unit u_csr (
        ...
        .retire_en(mem_retire), .branch_en(ex_branch), .mispredict_en(ex_mispredict)
    );
