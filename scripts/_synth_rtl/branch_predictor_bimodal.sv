// Bimodal predictor + direct-mapped BTB + 4-entry return address stack.
// Bimodal: PHT indexed by PC[N+1:2] only (no global history).
// BTB and RAS identical to gshare variant.
// Same interface as branch_predictor.sv for drop-in replacement.
...
module branch_predictor_bimodal #(
  ...
  // PHT: 2-bit saturating counters, indexed by PC only (NO GHR)
  logic [1:0] pht [0:PHT_DEPTH-1];

  // BTB (identical to gshare)
  ...
  // Prediction outputs (identical logic to gshare)
  always_comb begin
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      if (update_en) begin
        // NO GHR update (bimodal has no global history)

        // PHT update (same saturating counter logic)
