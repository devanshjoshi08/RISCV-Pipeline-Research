// 7-stage RV32IM pipeline: IF1 → IF2 → ID → EX1 → EX2 → MEM → WB
// IF is split into IF1 (PC gen, icache read) and IF2 (branch prediction).
// The icache output is registered in pipe_if1_if2, decoupling BRAM timing
// from PC generation for higher Fmax on Artix-7.
...
// Forwarding: 3 sources (EX2 → EX1, MEM → EX1, WB → EX1), same as 6-stage.
// MDU remains multi-cycle (2-cycle multiply, 32-cycle divide), stalls pipeline.

import pkg_riscv::*;

// COMBINED VARIANT: SGF (dual-GHR speculative forwarding) + NLP (next-line
// predictor in IF1). Tests orthogonality: SGF removes Mechanism-B
// mispredictions; NLP removes the IF2 fetch bubble. The NLP shadows the SGF
// predictor's IF2 output and touches no GHR/checkpoint state.
module rv32i_pipeline_7stage_sgf_nlp_top (
  input logic clk,
  input logic rst_n,
  ...
  output logic [31:0] debug_alu_result
    ...
    // SGF predictor's IF2 (real) prediction
    logic if2_real_redirect;
    assign if2_real_redirect = if2_predict_valid && if2_predict_taken;

    // NLP metadata carried IF1 -> IF2
    logic        if2_nlp_redir;
    logic [31:0] if2_nlp_tgt;

    // IF2 verification of the NLP action
    logic        if2_agree, if2_correct;
    ...
    always_comb begin
        if (if2_nlp_redir) begin
            if (if2_agree)              begin if2_correct = 1'b0; if2_correct_tgt = 32'b0; end
            else if (if2_real_redirect) begin if2_correct = 1'b1; if2_correct_tgt = if2_predict_target; end
            else                        begin if2_correct = 1'b1; if2_correct_tgt = if2_pc_plus4; end
        end else begin
            if (if2_real_redirect)      begin if2_correct = 1'b1; if2_correct_tgt = if2_predict_target; end
            else                        begin if2_correct = 1'b0; if2_correct_tgt = 32'b0; end
        end
    end
    ...
    // Gate prediction-driven redirects during stalls (as in the baseline).
    logic if2_correct_g;
    assign if2_correct_g = if2_correct && !pc_stall;
    logic nlp_drive;
    assign nlp_drive = nlp_hit_if1 && !ex2_redirect && !if2_correct_g && !pc_stall;

    // Repurpose if2_redirect as the IF2-correction flush so the downstream SGF
    // checkpoint-forwarding / if1_if2_flush logic remains unchanged.
    assign if2_redirect = if2_correct_g;
    ...
    // NLP metadata register: mirrors pipe_if1_if2 stall/flush behaviour
    ...
    // NLP training: shadow the IF2 SGF/BTB taken prediction
