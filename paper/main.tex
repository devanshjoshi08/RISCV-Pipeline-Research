...
\begin{abstract}
Deeper FPGA pipelines inflate branch misprediction counts because the global history register (GHR) becomes stale before it is updated.
We isolate this effect on Xilinx Artix-7 using five RV32IM pipeline variants (4--8 stages) that share all RTL except pipeline interconnect, and separate the CPI cost of depth into an inherent penalty-scaling component (Mechanism~A) and a correctable stale-GHR component (Mechanism~B).
Speculative GHR Forwarding (SGF) eliminates Mechanism~B by updating the GHR speculatively at prediction time with checkpoint-based rollback, reducing mispredictions by 31\% on CoreMark at 0.7\% area cost with no frequency loss.
\end{abstract}
...
Gonzalez and Horowitz identified this effect in ASIC power-performance simulations \cite{gonzalez1999power}, and high-performance ASIC out-of-order processors (e.g., Alpha~21264 \cite{kessler1999alpha}, MIPS~R10000 \cite{yeager1996mips}) deploy speculative history update mechanisms to address it.
...
SGF adapts the dual-GHR checkpointing concept from ASIC out-of-order processors to the constraints of in-order FPGA soft cores, where reorder buffers and deep checkpoint arrays are impractical.
...
On CoreMark, SGF pushes 7/8-stage misprediction counts below the 4/5-stage baseline (101K vs.\ 145K) because the speculative GHR reflects the most recent predicted control flow rather than waiting for resolution.
...
\begin{enumerate}
    ...
    \item \textbf{Stale-GHR degradation measurement on FPGA:} controlled separation of pipeline-depth CPI overhead into Mechanism~A (penalty scaling) and Mechanism~B (stale-GHR misprediction inflation), with per-workload classification across seven benchmarks (Section~\ref{sec:mispred}).
...
Seznec and Michaud studied the interaction between predictor update latency and table aliasing in conditional branch predictors \cite{michaud1999trading}.
Seznec later proposed the TAGE predictor \cite{seznec2011tage}, which uses multiple history lengths.
Speculative history update mechanisms are standard in high-performance ASIC out-of-order processors (Alpha~21264 \cite{kessler1999alpha}, MIPS~R10000 \cite{yeager1996mips}), where deep reorder buffers provide checkpoint storage.
...
Two distinct gaps exist in the literature.
The \emph{measurement gap}: no controlled experiment varies only pipeline depth while holding all other microarchitectural variables constant on a single FPGA device.
The \emph{mechanism gap}: no lightweight, in-order-compatible fix for stale-GHR degradation has been proposed for FPGA soft cores, where the ASIC solutions (deep checkpoint arrays, reorder-buffer-backed speculative history) are impractical.
...
Pipeline depth is not a perfectly clean independent variable: each split introduces structural side effects (load-use stall changes, IF2 bubbles, forwarding-source count changes) that are inseparable from the depth increase itself.
Table~\ref{tab:hazard} makes these asymmetries explicit.
The analytical CPI model (Section~\ref{sec:model}) addresses this by decomposing CPI into separate terms for each structural effect, allowing the contribution of depth \emph{per se} to be distinguished from the contribution of the side effects each split introduces.
The key claim is not that ``depth alone causes CPI increase'' but that ``each split introduces a quantifiable combination of frequency benefit and CPI cost, and the Mechanism~B component of that cost is correctable.''
...
Table~\ref{tab:embench} presents four official Embench-IoT \cite{embench2019} benchmarks.
Each was compiled from the official source with our bare-metal porting layer.
...
Workload base ($\gamma_w$)         & 0.45  & 0.45  & 0.45  & 0.45  & 0.45 \\
\midrule
...
The explanation is straightforward: the speculative GHR has zero update latency (it incorporates the predicted direction in the same cycle as the prediction), while even the shallowest baseline has 2-stage latency.
When predictions are mostly correct (79.8\% on CoreMark), the speculative GHR approximates true branch history more accurately than any committed GHR.
...
SGF targets GHR-based predictors (gshare \cite{mcfarling1993combining}, TAGE \cite{seznec2011tage}, and similar correlation predictors).
...
Extension to tournament or TAGE predictors \cite{seznec2011tage}, which maintain multiple history lengths, would require per-table speculative histories.
...
The controlled depth sweep that motivated SGF revealed a two-tier frequency structure on Artix-7 ($p < 10^{-48}$) and separated CPI overhead into Mechanism~A (inherent) and Mechanism~B (correctable).
A first-principles CPI model ($R^2 = 0.977$) decomposes each component.
...
Future work includes extending SGF to tournament and TAGE predictor architectures \cite{seznec2011tage}, where multiple history lengths require per-table speculative state, and validating the two-tier frequency structure and SGF benefits on Intel FPGA fabrics.
